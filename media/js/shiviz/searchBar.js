...
function SearchBar() {
  if (SearchBar.instance)
    throw new Exception(
      "Cannot instantiate SearchBar, instance already exists"
    );
  ...
  this.graphBuilder.setUpdateCallback(function () {
    ...
    var vts = new VectorTimestampSerializer(
  ...
  $("#searchbar #bar input")
    ...
    .on("keydown.search", function (e) {
      ...
      if (!context.isPanelShown()) return;
  ...
  $("#searchButton").on("click", function (e) {
    ...
    if (e.ctrlKey && e.altKey) {
      var regexp =
        '(?<event>){"host":"(?<host>[^}]+)","clock":(?<clock>{[^}]*})}';
      Shiviz.getInstance().visualize(
        context.getValue(),
        regexp,
        "",
        "order",
  ...
  $("#motifOption input").on("change", function () {
    if (
      $(this).is(":checked") ||
      $(this).siblings("input:checkbox:checked").length > 0
    ) {
...
SearchBar.prototype.showPanel = function () {
  ...
  $(window).on("mousedown", function (e) {
    var $target = $(e.target);
    if (!$target.parents("#searchbar").length) context.hidePanel();
  });
};
...
SearchBar.prototype.showSearchHistory = function () {
  ...
  if (context.storageAvailable()) {
    ...
    history.reverse().forEach((previousSearch) => {
      searchHistoryTab.append(
        '<dt class="historyItem"><code>' + previousSearch + "</code></dt>"
      );
    });
...
SearchBar.prototype.clearResults = function () {
  ...
  if (this.global != null && this.global.getController().hasHighlight()) {
    ...
    if (
      this.global.getViews().length > 1 &&
      !$(".leftTabLinks li").first().next().hasClass("default") &&
      !$(".pairwiseButton").is(":visible")
    ) {
      $(".pairwiseButton").show();
    }
  }
    ...
    switch (this.mode) {
      ...
      case SearchBar.MODE_MOTIF:
        ...
        $.get(url, function (response) {
          ...
          prefix =
            "https://api.github.com/repos/bestchai/shiviz-logs/contents/";
          ...
          $.get(url, function (response) {
            ...
            Shiviz.getInstance().handleException(
              new Exception("unable to retrieve motifs from: " + url, true)
            );
          });
        });
        break;
...
SearchBar.prototype.countMotifs = function () {
  ...
  if ($("#searchbar").hasClass("results")) {
    ...
    this.motifNavigator.addMotif(
      views[0].getVisualModel(),
      views[0].getTransformer().getHighlightedMotif()
    );
    if (this.global.getPairwiseView()) {
      this.motifNavigator.addMotif(
        views[1].getVisualModel(),
        views[1].getTransformer().getHighlightedMotif()
      );
    }
