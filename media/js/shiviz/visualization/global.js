...
Global.prototype.drawAll = function () {
  ...
  if (
    $(".leftTabLinks li").is(":visible") &&
    $(".leftTabLinks li").last().hasClass("default")
  ) {
    if (
      $("#clusterNumProcess").is(":checked") ||
      ($("#clusterComparison").is(":checked") &&
        $(".clusterBase").find("option:selected").text() !=
          "Select a base execution")
    ) {
      labelIconL.show();
      selectIconL.show();
  ...
  if (this.viewR != null) {
    ...
    if (
      !$(".leftTabLinks li").first().next().hasClass("default") &&
      searchbar.getMode() != SearchBar.MODE_MOTIF
    ) {
      $(".pairwiseButton").show();
    }
    ...
    if (numViews == 2 && global.getPairwiseView()) {
      ...
      viewSelectL
        .children("option[value='" + leftLabel + "']")
        .prop("selected", true);
      viewSelectR
        .children("option[value='" + rightLabel + "']")
        .prop("selected", true);
  ...
  // If there is a single execution
    ...
    // * PeasyViz Change - Adjusted the height of the empty div
    // $(".visualization .left #tabs").css("height", "2.5em");
    $(".visualization .left #tabs").css("height", "6em");
  }
  ...
  if (this.getPairwiseView()) {
    ...
    if (
      this.viewL.getTransformer().getHiddenHosts().length <
        this.viewL.getHosts().length &&
      this.viewR.getTransformer().getHiddenHosts().length <
        this.viewR.getHosts().length
    ) {
    ...
    this.controller.bindLines(
      this.viewR.getLogTable().find(".line:not(.more)")
    );
  }
...
Global.prototype.setView = function (position, anchorHref) {
  ...
  if (
    (position == "L" && anchorHref == rightView) ||
    (position == "R" && this.getPairwiseView() && anchorHref == leftView)
  ) {
    this.swapViews();
  } else {
    ...
    if (this.getPairwiseView() && this.getViews().length == 2) {
      ...
      if (position == "L") {
        $("#viewSelectL")
          .children("option[value='" + anchorHref + "']")
          .prop("selected", true)
          .change();
      } else {
        $("#viewSelectR")
...
Global.prototype.resize = function () {
  var viewLNumHosts = getNumVisibleHosts(
    this.viewL.getHosts(),
    this.viewL.getTransformer().getSpecifiedHiddenHosts()
  );

  var viewRNumHosts = 0;
  if (this.viewR != null && this.getPairwiseView()) {
    viewRNumHosts = getNumVisibleHosts(
      this.viewR.getHosts(),
      this.viewR.getTransformer().getSpecifiedHiddenHosts()
    );
  }
  ...
  // * PeasyViz Change - Decreased the width of the searchbar because it overlapped with right sidebar texts, added '- 64' to the globalWidth
  $("#searchbar").width(globalWidth - 20);

  var widthPerHost = Math.max(
    Global.MIN_HOST_WIDTH,
    globalWidth / visibleHosts
  );
  var logTableWidth =
...
Global.prototype.drawClusterIcons = function () {
  $(
    "#clusterIconL, #clusterIconR, br.spaceL, br.spaceR, br.left, br.right"
  ).remove();
  ...
  var leftLink = $("table.clusterResults a").filter(function () {
    return $(this).attr("href") == leftLabel;
  });
  var rightLink = $("table.clusterResults a").filter(function () {
  ...
  if ($("#clusterComparison").is(":checked")) {
    if (leftLabel == $("select.clusterBase").val()) {
      ...
      $(".clusterBase")
        .before(clusterIconL.addClass("baseIcon"))
        .addClass("baseIndent");
      if (this.getPairwiseView()) {
        $(rightLink.before(clusterIconR).next()).after(
          $("<br class=right>").hide()
        );
      }
      // If the right graph is the specified base execution, draw the right arrow icon next to the dropdown
    } else if (
      this.getPairwiseView() &&
      rightLabel == $("select.clusterBase").val()
    ) {
      $("#baseLabel").after($("<br class='spaceR'>").hide());
      $(".clusterBase")
        .before(clusterIconR.addClass("baseIcon"))
        .addClass("baseIndent");
      $(leftLink.before(clusterIconL).next()).after(
        $("<br class=left>").hide()
      );
    }
  }
...
Global.prototype.drawSideBar = function () {
  ...
  hiddenHostsSVG
    ...
    .attr(
      "height",
      Math.ceil(hh.length / hostsPerLine) * (Global.HOST_SIZE + 5) - 5
    )
