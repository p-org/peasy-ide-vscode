...
function Controller(global) {
  ...
  $(window)
    ...
    .on("click.dialog", function (e) {
      ...
      if (
        tn == "g" ||
        $target.parents("g").length ||
        $target.parents(".hidden-hosts").length
      )
        return;
      // Test for line click
  ...
  $(".dialog button")
    ...
    .click(function () {
      ...
      switch (type) {
        ...
        // Toggle the fields for a node.
        case "toggle fields":
          self.toggleShowFields();
          break;
      }
      self.bindScroll();
    });
  ...
  $(".visualization .leftTabLinks a")
    .unbind()
    .on("click", function (e) {
      var anchorHref = $(this).attr("href");
      $(".visualization #" + anchorHref)
        .show()
        .siblings()
        .hide();
      $(this)
  ...
  $("#clusterNumProcess, #clusterComparison")
    .unbind()
    .on("change", function () {
      $("#labelIconL, #labelIconR, #selectIconL, #selectIconR").hide();
...
/**
 * Toggles the show/unshow the fields of a node.
 */
Controller.prototype.toggleShowFields = function () {
  $(window).unbind("scroll");
  $dialog = $(".dialog");
  this.fieldsIsExpanded = !this.fieldsIsExpanded;
  // Show info table is show more, else hide
  if (this.fieldsIsExpanded) {
    $dialog.find(".info").show();
    $dialog.find(".show-fields").show().text("Show Less");
  } else {
    $dialog.find(".info").hide();
    $dialog.find(".show-fields").show().text("Show More");
  }
  this.bindScroll();
};
      ...
      if ($(".leftTabLinks li").first().hasClass("default")) {
        ...
        $(".highlight")
          .css({
            ...
            left:
              $line.offset().left -
              parseFloat($line.css("margin-left")) -
              vleft,
          })
          .attr({
...
Controller.prototype.onScroll = function (e) {
  ...
  if ($(".line.focus").length) {
    $(".highlight").css({
      left:
        $(".line.focus").offset().left -
        parseFloat($(".line.focus").css("margin-left")) -
        $(".visualization .left").offset().left,
    });
  }
  ...
  if (!type) {
    ...
    if (this.global.getShowDiff()) {
      ...
      if (uniqueEventsL.indexOf(id) == -1 && uniqueEventsR.indexOf(id) == -1) {
        ...
        selcirc.attr("class", "sel").attr("r", function (d) {
          return d.getRadius() + 6;
        });
        ...
        selrhombus
          ...
          .style("fill", "white");
        selrhombus.attr("class", "sel").attr("points", function (d) {
