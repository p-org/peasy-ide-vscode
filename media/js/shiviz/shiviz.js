/****************************************************************
 * * Importing TRex parser, and helper functions to convert     *
 * * raw P error trace inputs into a compatiable one for ShiViz *
 ****************************************************************/
// var parseTxtTrace = require("../util/trexParser");
// var generateShiVizCompatiableInput = require("../util/generateShiVizCompatibleInput");
...
function Shiviz() {
    ...
    $.get(url, function (response) {
      ...
      $.get(url, function (response) {
        ...
        Shiviz.getInstance().handleException(
          new Exception(
            "Unable to retrieve example log from:\n" +
              url +
              "\n\n Note: to use ShiViz example logs offline in Chrome, start Chrome with:\n $ open -n -a '/Applications/Google Chrome.app/' --args --allow-file-access-from-files",
            true
          )
        );
      });
  ...
  // * Set default value of parser input to be one that's compatibale with P
  $("#parser").attr(
    "value",
    "(?<event>[^\\n]+)\\n(?<host>[^\\n]+)\\n(?<clock>[^\\n]+)"
  );
  ...
  $(window).on("popstate", function (e) {
    context.go(
      e.originalEvent.state == null ? 0 : e.originalEvent.state.index,
      false
    );
  });
