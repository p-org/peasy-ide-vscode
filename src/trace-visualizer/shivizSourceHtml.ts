...
const shivizSourceHtml = (
  ...
  return `<!DOCTYPE html>
	...
	<head>
		...
		<script src="https://kit.fontawesome.com/e408c4557f.js" crossorigin="anonymous"></script>
		<style type="text/css">
	...
	<body>
		...
		<section class="home">
			...
			<div class="content">
				<h1>ShiViz</h1>
				<h2>
					The <strong>ShiViz</strong> vis&shy;ual&shy;izat&shy;ion engi&shy;ne gene&shy;r&shy;ate&shy;s
					inter&shy;active comm&shy;uni&shy;ca&shy;tion graphs from dis&shy;tri&shy;but&shy;ed sys&shy;tem
					exe&shy;cut&shy;ion logs.
				</h2>
				...
				<ul>
					...
					<li>
						The left panel shows the log and the middle panel displays a <a
							href="http://en.wikipedia.org/wiki/Directed_acyclic_graph">DAG</a> of the partially ordered
						vector timestamps recorded in the input log.
					</li>
					<li>
						A vertical line with a box at the top is a <strong>process timeline</strong>. This represents a
						single thread of execution (e.g., a process).
					</li>
					<li>
					...
					<li>
						...
						<a href="http://en.wikipedia.org/wiki/Happened-before">happened-before</a> relation between the
						events: the higher of the two events happened <i>before</i> the second event.
					</li>
					<li>
						Dashed lines represent transitive communication edges. These only appear when a process is hidden,
						and two processes that are not hidden communicated indirectly through this process.
					</li>
					<li>
						Larger circles represent merged local events. These have a number inside of them, indicating the
						number of events that they represent.
					</li>
					<li>
						Radiating lines that fade out represent communication edges to processes that are currently hidden
						from view.
					</li>
					<li>
						Each process is associated with a unique color. Log entries in the left column have the
						corresponding process color. By default, the log entries are sorted in the order in which the
						entries appear in the graph.
					</li>
				</ul>
				</p>
				...
				<ul>
					...
					<li>
						...
						<br>
						Visualize concurrency and the happened-before relation to understand if two events occurred
						concurrently, or if one happened before the other.
					</li>
					<li>
						<strong>Analyzing activity at processes that matter</strong>.
						<br>
						Hide processes (and their log lines) for a more focused visualization. Filter by communication to a
						process to see only those events at other processes that were likely to have influenced the
						execution of a process you care about.
					</li>
				...
				<p>
					You can specify the log format using a regular expression on the Input page. The regular expression must
					contain the following three named capture groups:
				<ul>
					<li><strong><code>event</code>:</strong> the description of the event</li>
					<li><strong><code>host</code>:</strong> the name of the process, thread, or host that executed the event
					</li>
					<li><strong><code>clock</code>:</strong> the vector clock associated with the event, in JSON format</li>
				</ul>
				</p>
				<p>
					You can also specify other capture groups other than <code>event</code> to capture <em>fields</em> (such
					as date, IP, priority) which are shown in the sidebar while hovering over an event in the visualization.
					These fields, unless sub-groups of the <code>event</code> group, will not be displayed as part of the
					event message.
				</p>
				<p>
				...
				<ul>
					...
					<li><a href="http://homes.cs.washington.edu/~mernst/pubs/debug-distributed-cacm2016-abstract.html">How
							to cite</a></li>
				</ul>
			</div>
			...
			<table>
				<tr>
					<td class="left">
						...
						<div id="file_input">
							...
							<div id="file_desc">Upload a log by selecting a file with the following format: <br><br>
								<li> The first line of the file is the log parsing regular expression. If left empty, the
									default regular expression
									<span class="regex">"(?&ltevent&gt.*)\n(?&lthost&gt\S*) (?&ltclock&gt{.*})"</span> will
									be used.<br>
								<li> The second line of the file is the multiple executions regular expression delimiter.
									This can be left empty if the log does not contain multiple executions. <br>
						...
						<div class="option">
							<label>Log parsing regular expression:
								<div id="file_desc">Upload a log by selecting a file with the following format: <br><br>
									<li> The first line of the file is the log parsing regular expression. If left empty,
										the default regular expression
										<span class="regex">"(?&ltevent&gt.*)\n(?&lthost&gt\S*) (?&ltclock&gt{.*})"</span>
										will be used.<br>
									<li> The second line of the file is the multiple executions regular expression
										delimiter. This can be left empty if the log does not contain multiple executions.
										<br>
									<li> The rest of the file is the log.
								</div>
						...
						<div class="option">
							...
							<label><input id="hostsortOrder" name="host_sort" type="radio" checked value="order"> appearance
								in log</label>
						</div>
					...
					<td class="right">
						...
						<div id="examples">
							<b>Example logs:</b>
							<a class="log-link" href="" data-log="simple-reliable-broadcast.log" data-ordering="ascending"
								data-hostsort="#hostsortOrder"
								data-parser="\[\w+\] \[(?<date>([^ ]+ [^ ]+))\] [^ ]+ \[akka://Broadcast/user/(?<host>\w+)\] (?<clock>.*\}) (?<event>.*)">Reliable
								broadcast</a>
							<a class="log-link" href="" data-log="chord.log" data-ordering="descending" data-hostsort="#hostsortLength"
								data-parser="(?<host>\S*) (?<clock>{.*})\n(?<event>.*)">Chord DHT</a>
							<a class="log-link" href="" data-log="tsviz_fslock_24t_4sp.log" data-ordering="descending"
								data-hostsort="#hostsortLength"
								data-parser="(?<timestamp>(\d*)) (?<event>.*)\n(?<host>\w*) (?<clock>.*)">WiredTiger
								KV-store lock contention</a>
							<a class="log-link" href="" data-log="tsviz_shared_var_4_threads.log" data-ordering="descending"
								data-hostsort="#hostsortLength"
								data-parser="(?<timestamp>(\d*)) (?<event>.*)\n(?<host>\w*) (?<clock>.*)">WiredTiger shared
								variable contention</a>
							<a class="log-link" href="" data-log="voldemort-simple-threadnames.log"
								data-parser="\[(?<date>\d{4}-\d{2}-\d{2} (\d{2}:){2}\d{2},\d{3}) (?<path>\S*)\] (?<priority>(INFO|WARN)) (?<event>.*)\n(?<host>\S*) (?<clock>{.*})">Voldemort</a>
							<a class="log-link" href="" data-log="simpledb.log"
								data-parser="(?<event>.*)\n(?<host>\S*) (?<clock>{.*})">SimpleDB</a>
							<a class="log-link" href="" data-log="CPSC416-2018W1-P2-Slack/execution2.log"
								data-parser="(?<host>\S*) (?<clock>{.*})\n(?<event>.*)">Distributed slack</a>
							<a class="log-link" href="" data-log="CPSC416-2018W1-P1-RFS/execution3.log"
								data-parser="(?<host>\S*) (?<clock>{.*})\n(?<event>.*)">RecordsFS</a>
							<a class="log-link" href="" data-log="CPSC416-2017W2-P2-Twittor/execution3.log"
								data-parser="(?<host>\S*) (?<clock>{.*})\n(?<event>.*)">TwitToR</a>
							<a class="log-link" href="" data-log="facebook.log"
								data-parser="(?<ip>(\d{1,3}\.){3}\d{1,3}) (?<date>(\d{1,2}/){2}\d{4} (\d{2}:){2}\d{2} (AM|PM)) (?<action>(INFO|GET|POST)) (?<event>.*)\n(?<host>\w*) (?<clock>.*)">Data-center
								lad balancer (synth)</a>
							<a class="log-link" href="" data-log="facebook-multiple.log" data-delimiter="^=== (?<trace>.*) ===$"
								data-parser="(?<ip>(\d{1,3}\.){3}\d{1,3}) (?<date>(\d{1,2}/){2}\d{4} (\d{2}:){2}\d{2} (AM|PM)) (?<action>(INFO|GET|POST)) (?<event>.*)\n(?<host>\w*) (?<clock>.*)">Multi-execution
								(synth)</a>
							<a class="log-link" href="" data-log="multiple-comparison.log" data-delimiter="^=== (?<trace>.*) ===$"
								data-parser="(?<ip>(\d{1,3}\.){3}\d{1,3}) (?<date>(\d{1,2}/){2}\d{4} (\d{2}:){2}\d{2} (AM|PM)) (?<action>(INFO|GET|POST)) (?<event>.*)\n(?<host>\w*) (?<clock>.*)">Comparison
								log (synth)</a>
							<a class="log-link" href="" data-log="ewd998.log" data-delimiter="^=== (?<trace>.*) ===$"
								data-parser='^State [0-9]+: <(?<event>\w*) .*>\n\/\\ Host = (?<host>.*)\n\/\\ Clock = "(?<clock>.*)"\n\/\\ active = (?<active>.*)\n\/\\ color = (?<color>.*)\n\/\\ counter = (?<counter>.*)'>EWD998
								from TLA+</a>
						</div>
		...
		<section class="visualization">
			<div id="panel-toggle">
				<div class="panel-toggle-options selected" id="panel-toggle-all" data-panel-selection="all">All</div>
				<div class="panel-toggle-options" id="panel-toggle-logs" data-panel-selection="logs">Logs</div>
				<div class="panel-toggle-options" id="panel-toggle-graph" data-panel-selection="graph">Graph</div>
			</div>
			<div class="left">
				<!-- <div class="bg"></div> -->
						...
						<div id="textTab" class="tab">
							...
							<div class="out">
								...
								<dl>
									...
									<dd>Find events with "priority" field set to "CRITICAL" and with "text" field matching
										the specified regex. Supported logical operators are: <code>&amp;&amp;</code>,
										<code>||</code>, <code>^</code>
									</dd>
									<dt><code>ip="216.58.216.174"</code></dt>
									<dd>String literals containing non-alphanumeric symbols must be surrounded by quotes.
									</dd>
