...
function LogParser(rawString, delimiter, regexp) {
  ...
  if (
    ...
    names.indexOf("event") < 0
  ) {
    var e = new Exception(
      "The parser RegExp you entered does not have the necessary named capture groups.\n",
      true
    );
    ...
    for (var i = 0; i < currExecs.length; i++) {
      if (currExecs[i].trim().length > 0) {
        ...
        if (this.executions[currlabel]) {
          throw new Exception(
            'Execution names must be unique. There are multiple executions called "' +
              currlabel +
              '"',
            true
          );
        }
        this.executions[currlabel] = new ExecutionParser(
          currExecs[i],
          currlabel,
          regexp
        );
...
function ExecutionParser(rawString, label, regexp) {
  ...
  while ((match = regexp.exec(rawString))) {
    ...
    regexp.getNames().forEach(function (name, i) {
      if (name == "clock" || name == "event") return;
    ...
    try {
      ...
      try {
        ...
        var exception = new Exception(
          "An error occured while trying to parse the vector timestamp on line " +
            (line + 1) +
            ":"
        );
