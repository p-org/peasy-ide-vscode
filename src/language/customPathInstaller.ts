...
export class CustomPathInstaller {
  ...
  public async getExecutable(context: ExtensionContext, server: boolean, newArgs: string[]): Promise<Executable | undefined> {
  ...
  private async getCliPathUncached(context: ExtensionContext): Promise<string> {
    let cliPathOverride = process.env['P_SERVER_OVERRIDE'] ?? '';
    ...
    if(cliPathOverride || cliPath) {
      const originalPath = cliPathOverride || cliPath;
      let acopyofP = originalPath;
      try {
        cliPathOverride = await this.copyNecessaryDllsToTempFolder(originalPath);
        acopyofP = `a copy of ${originalPath} in ${cliPathOverride}`;
      } catch(e: unknown) {
        // Ignore if copying fails. We'll run the original.
      }
      window.showInformationMessage(`Using ${acopyofP} as P executable`);
    }
    if(cliPathOverride) {
  ...
  // We copy P.dll and all its dependencies to another location
  // so that rebuilding P will not fail because it's open in VSCode
  private async copyNecessaryDllsToTempFolder(configuredPath: string): Promise<string> {
    ...
    const vscodeDir = path.join(os.tmpdir(), await fs.promises.mkdtemp('vscode-p-dlls-'));
